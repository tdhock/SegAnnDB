...
function profilePlot(rowList){ 
  ...
  for(var i=0; i < rowList.length; i++){
    ...
    var svgs = tds.append("svg")
      .attr("id",function(d){return "pro"+d.profile+"chrom"+d.chr;})
      .attr("width",function(d){return d.width_px})
...
function chromDisplay(
  ...
  function annotationTrack(chromD,trackY,trackHeight,trackType){
	...
	if(cropL == cropR){ //both outside range
	  rect.remove();
	  return;
	}
	// abort if too small.
	...
	var x_space = 2;
	var buttons = [
	  {"sign":-1,"name":"Delete","anchor":"end",
	   "fun":deleteAnnotation},
	  {"sign":1,"name":"Save","anchor":"start",
	   "fun":saveAnnotation},
	];
    ...
    function rectActions(rects){
      rects
	.attr("class",trackType)
	.attr("id",function(d){return trackType+d.id;})
	.style("opacity",0.7)
	.on("click",function(d){
	  if(d3.event.shiftKey){//zoom to ucsc.
	  ...
	  }else{// normal click, delete the annotation.
	...
	.style("fill",function(d){return trackColors[d.annotation];})
	...
	.style("text-anchor","middle")
	.style("font-weight","bold")
	.attr("id",function(d){return trackType+d.id+"label";})
      ;
      rectActions(rects);
    }
  }
  ...
  this.updateModel = function(response){
	...
	.data([{"logratio":0},{"logratio":-1},{"logratio":1}])
      ;
      var guideActions = function(selection){
	selection.attr("x1",0)
	  .attr("x2",width)
	  .attr("y1",function(d){return y(d.logratio);})
	  .attr("y2",function(d){return y(d.logratio);})
	  .classed("guide",1)
	  .style("stroke-width",function(d){
	    if(d.logratio)return "1px";
	    else return "3px";
	  })
	;
      }
	...
	.data(response.segments);
      ...
      segmentation
	.attr("x1",function(d){return x(d.min-0.5);})
	...
	.data(response.breakpoints);
      ...
      breakpoints
	.attr("x1",function(d){return x(d.position+0.5);})
	.attr("x2",function(d){return x(d.position+0.5);})
